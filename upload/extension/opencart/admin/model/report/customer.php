...
class Customer extends \Opencart\System\Engine\Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT c.customer_id, CONCAT(c.firstname, ' ', c.lastname) AS customer, c.email, cgd.name AS customer_group, c.status, `o`.`order_id`, SUM(op.quantity) AS products, `o`.total AS total, `o`.`currency_code`, `o`.`currency_value`, (SELECT oca.`affiliate_id` FROM `" . DB_PREFIX . "order_customer_affiliate` oca WHERE oca.`order_id` = `o`.`order_id` GROUP BY oca.`order_id`) AS affiliate_id, SUM((SELECT SUM(oca.`commission`) FROM `" . DB_PREFIX . "order_customer_affiliate` oca WHERE oca.`order_id` = `o`.`order_id` GROUP BY oca.`order_id`)) AS commission FROM `" . DB_PREFIX . "order` `o` LEFT JOIN `" . DB_PREFIX . "order_product` op ON (`o`.`order_id` = op.`order_id`) LEFT JOIN `" . DB_PREFIX . "customer` c ON (o.customer_id = c.customer_id) LEFT JOIN `" . DB_PREFIX . "customer_group_description` cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE o.customer_id > 0 AND cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (!empty($data['filter_affiliate'])) {
			$sql .= " AND `o`.`order_id` IN (SELECT oca.`order_id` FROM `" . DB_PREFIX . "order_customer_affiliate` oca WHERE CONCAT(oca.`firstname`, ' ', oca.`lastname`) LIKE '" . $this->db->escape($data['filter_affiliate']) . "')";
		}
