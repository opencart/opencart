...
class FraudLabsPro extends \Opencart\System\Engine\Model {
	public function install() {
		...
		BEGIN
			IF NOT EXISTS(SELECT NULL FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'oc_fraudlabspro' AND COLUMN_NAME = 'fraudlabspro_rules')
			THEN ALTER TABLE `oc_fraudlabspro` ADD `fraudlabspro_rules` VARCHAR(200) NOT NULL;
			END IF;
		END;");
	...
	public function uninstall() {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `code` = 'fraudlabspro'");
	}
