...
class Tag extends \Opencart\System\Engine\Controller {
	...
	public function delete(array $args = []): array {
		...
		$file = DIR_OPENCART . 'shop/' . parse_url($store_info['url'], PHP_URL_HOST) . '/data/tag/filter_group-' . (int)$args['filter_group_id'] . '.json';

		if (is_file($file)) {
			unlink($file);
		}

		$file = DIR_OPENCART . 'shop/' . parse_url($store_info['url'], PHP_URL_HOST) . '/data/tag/filter-product-' . (int)$args['filter_group_id'] . '.json';
