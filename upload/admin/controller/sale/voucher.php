...
class ControllerSaleVoucher extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->ssl('sale/voucher/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->ssl('sale/voucher/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			if ($result['order_id']) {	
				$order = $this->url->ssl('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, true);
			} else {
				$order = '';
			}
			...
			$data['vouchers'][] = array(
				...
				'edit'       => $this->url->ssl('sale/voucher/edit', 'token=' . $this->session->data['token'] . '&voucher_id=' . $result['voucher_id'] . $url, true),
				'order'      => $order
			);
		}
		...
		$data['sort_code'] = $this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.code' . $url, true);
		$data['sort_from'] = $this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.from_name' . $url, true);
		$data['sort_to'] = $this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.to_name' . $url, true);
		$data['sort_theme'] = $this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=theme' . $url, true);
		$data['sort_amount'] = $this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.amount' . $url, true);
		$data['sort_status'] = $this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.date_end' . $url, true);
		$data['sort_date_added'] = $this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=v.date_added' . $url, true);
		...
		$pagination->url = $this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['voucher_id'])) {
			$data['action'] = $this->url->ssl('sale/voucher/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->ssl('sale/voucher/edit', 'token=' . $this->session->data['token'] . '&voucher_id=' . $this->request->get['voucher_id'] . $url, true);
		}

		$data['cancel'] = $this->url->ssl('sale/voucher', 'token=' . $this->session->data['token'] . $url, true);
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $voucher_id) {
			...
			if ($order_voucher_info) {
				$this->error['warning'] = sprintf($this->language->get('error_order'), $this->url->ssl('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $order_voucher_info['order_id'], true));
	...
	public function history() {
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->ssl('sale/voucher/history', 'token=' . $this->session->data['token'] . '&voucher_id=' . $this->request->get['voucher_id'] . '&page={page}', true);
