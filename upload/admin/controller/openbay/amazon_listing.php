...
class ControllerOpenbayAmazonListing extends Controller {
	public function create() {
		...
		if ($this->request->post) {
			...
			if ($result['status'] === 1) {
				...
				$this->response->redirect($this->url->ssl('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->post['product_id'] . $url, true));
			}
		}
		...
		if (isset($this->request->get['product_id'])) {
			...
			if ($listing_status === 'processing' || $listing_status === 'ok') {
				$this->response->redirect($this->url->ssl('openbay/amazon_listing/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, true));
			} else if ($listing_status === 'error_advanced' || $listing_status === 'saved' || $listing_status === 'error_few') {
				$this->response->redirect($this->url->ssl('openbay/amazon_product', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, true));
			}
		} else {
			$this->response->redirect($this->url->ssl('extension/openbay/items', 'token=' . $this->session->data['token'] . $url, true));
		}
		...
		$data['url_search']  = $this->url->ssl('openbay/amazon_listing/search', 'token=' . $this->session->data['token'], true);
		$data['url_advanced']  = $this->url->ssl('openbay/amazon_product', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, true);
		...
		$data['form_action'] = $this->url->ssl('openbay/amazon_listing/create', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'href' => $this->url->ssl('openbay/amazon_listing/create', 'token=' . $this->session->data['token'], true),
	...
	public function edit() {
		...
		$data['breadcrumbs'][] = array(
			'href' => $this->url->ssl('openbay/amazon_listing/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $product_id . $url, true),
		...
		if ($status === false) {
			$this->response->redirect($this->url->ssl('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&product_id=' . $product_id . $url, true));
			return;
		}
		...
		if ($status == 'ok' || $status == 'linked') {
			$data['url_create_new']  = $this->url->ssl('openbay/amazon_listing/createNew', 'token=' . $this->session->data['token'] . '&product_id=' . $product_id . $url, true);
			$data['url_delete_links']  = $this->url->ssl('openbay/amazon_listing/deleteLinks', 'token=' . $this->session->data['token'] . '&product_id=' . $product_id . $url, true);
		}
		...
		$data['url_saved_listings']  = $this->url->ssl('openbay/amazon/savedListings', 'token=' . $this->session->data['token'] . '&product_id=' . $product_id, true);
