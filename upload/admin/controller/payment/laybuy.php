...
class ControllerPaymentLaybuy extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'], 'SSL')
		);
		...
		foreach ($results as $result) {
			...
			if ($customer_id) {
				$customer_url = $this->url->link('customer/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . (int)$customer_id, 'SSL');
			}

			$data['reports'][] = array(
				'id'			=> $result['laybuy_transaction_id'],
				'order_id'		=> $result['order_id'],
				'order_url'		=> $this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$result['order_id'], 'SSL'),
				...
				'view'			=> $this->url->link('payment/laybuy/transaction', 'token=' . $this->session->data['token'] . '&id=' . (int)$result['laybuy_transaction_id'], 'SSL')
			);
		}
		...
		$data['sort_order_id'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '&sort=lt.order_id' . $url . '#reportstab', 'SSL');
		$data['sort_customer'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '&sort=customer' . $url . '#reportstab', 'SSL');
		$data['sort_amount'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '&sort=lt.amount' . $url . '#reportstab', 'SSL');
		$data['sort_dp_percent'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '&sort=lt.downpayment' . $url . '#reportstab', 'SSL');
		$data['sort_months'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '&sort=lt.months' . $url . '#reportstab', 'SSL');
		$data['sort_dp_amount'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '&sort=lt.downpayment_amount' . $url . '#reportstab', 'SSL');
		$data['sort_first_payment'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '&sort=lt.first_payment_due' . $url . '#reportstab', 'SSL');
		$data['sort_last_payment'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '&sort=lt.last_payment_due' . $url . '#reportstab', 'SSL');
		$data['sort_status'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '&sort=lt.status' . $url . '#reportstab', 'SSL');
		$data['sort_date_added'] = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '&sort=lt.date_added' . $url . '#reportstab', 'SSL');
		...
		$pagination->url = $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . $url . '&page={page}#reportstab', 'SSL');
	...
	public function fetch() {
		...
		if ($this->user->hasPermission('modify', 'payment/laybuy')) {
			...
			if ($paypal_profile_id_array) {
				...
				$this->response->redirect($this->url->link('payment/laybuy', 'token=' . $this->session->data['token'], 'SSL'));
			} else {
	...
	public function transaction($order_page = false) {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('payment/laybuy', 'token=' . $this->session->data['token'] . '#reportstab', 'SSL')
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_transaction_title'),
			'href' => $this->url->link('payment/laybuy/transaction', 'token=' . $this->session->data['token'] . '&id=' . $id, 'SSL')
		);
		...
		if ($transaction_info) {
			...
			$data['transaction'] = array(
				...
				'remaining'        	=> $this->currency->format($this->model_payment_laybuy->getRemainingAmount($transaction_info['amount'], $transaction_info['downpayment_amount'], $transaction_info['payment_amounts'], $transaction_info['transaction'])),
