...
class Installer extends \Opencart\System\Engine\Controller {
	...
	public function upgrade(): void {
		...
		if (!$json) {
			...
			if (($page * 200) <= $total) {
				...
				$json['next'] = $this->url->link('marketplace/installer|vendor', 'user_token=' . $this->session->data['user_token'] . '&action=upgrade' . $url, true);
				$json['progress'] = 100;
			}
		}
	...
	public function downgrade(): void {
		...
		if (!$json) {
			...
			if (($page * 200) <= $total) {
				...
				$json['next'] = $this->url->link('marketplace/installer|vendor', 'user_token=' . $this->session->data['user_token'] . '&action=downgrade' . $url, true);
				$json['progress'] = 100;
			}
		}
	...
	private function addExtensionFilesAndPaths(array $extension_install_info, string $file, int $page, object $zip, string &$install_folder, array &$json): void {
		...
		if ($install_info) {
			...
			if(isset($this->request->get['install_folder'])) {
				$install_folder = $this->request->get['install_folder'];
			} else {
				for ($i = 0; $i < $zip->numFiles; $i++) {
					$path = $zip->getNameIndex($i);
