		...
		if (isset($promotion['extensions'])) {
			...
			foreach ($promotion['extensions'] as $result) {
				...
				if (!$extension_install_info) {
					$download = $this->url->link('marketplace/marketplace.download', 'extension_id=' . $result['extension_id'] . '&extension_download_id=' . $result['extension_download_id']);
				} else {
					$download = '';
				}
				...
				if ($extension_install_info && !$extension_install_info['status']) {
					$install = $this->url->link('marketplace/installer.install', 'extension_install_id=' . $extension_install_info['extension_install_id']);
				} else {
					$install = '';
				}
				...
				// Delete
				if ($extension_install_info && !$extension_install_info['status']) {
					$delete = $this->url->link('marketplace/installer.delete', 'extension_install_id=' . $extension_install_info['extension_install_id']);
				} else {
					$delete = '';
				}
				...
				if (!$extension_install_info || !$extension_install_info['status']) {
					$data['extensions'][] = [
						'name'     => $result['name'],
						'href'     => $this->url->link('marketplace/marketplace.info', 'extension_id=' . $result['extension_id']),
						'download' => $download,
						'install'  => $install,
						'delete'   => $delete
					];
