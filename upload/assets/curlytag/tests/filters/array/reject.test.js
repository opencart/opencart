...
describe('reject', () => {
    test('excludes items by truthy key', () => {
        ...
        expect(template.parse('{{ items | reject: "active" | length }}', data)).toBe('2');
    });

    test('returns empty array that is chainable', () => {
        const data = { items: [{ active: true }, { active: true }] };
        expect(template.parse('{{ items | reject: "active" | join: "," }}', data)).toBe('');
    });
    ...
    test('keeps items where key is missing', () => {
        const data = { items: [{ active: true }, {}, { active: true }] };
        expect(template.parse('{{ items | reject: "active" | length }}', data)).toBe('1');
    });
    ...
    test('keeps items where key is zero', () => {
        ...
        expect(template.parse('{{ items | reject: "stock" | length }}', data)).toBe('2');
    });

    test('keeps items with falsy empty string value', () => {
        const data = { items: [{ category: 'news' }, { category: '' }, { category: 'sport' }] };
        expect(template.parse('{{ items | reject: "category" | length }}', data)).toBe('1');
    });
