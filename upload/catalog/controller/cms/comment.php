...
class Comment extends \Opencart\System\Engine\Controller {
	...
	public function getList(): string {
		...
		foreach ($results as $result) {
			...
			$data['comments'][] = [
				...
				'reply'              => $reply, 'reply_add' => $this->url->link('cms/comment.add', 'language=' . $this->config->get('config_language') . '&comment_token=' . $this->session->data['comment_token'] . '&article_id=' . $article_id . '&parent_id=' . $result['article_comment_id']),
	...
	public function add(): void {
		...
		if (!$json) {
			...
			$comment_data = $this->request->post + ['parent_id' => $parent_id, 'ip' => $this->request->server['REMOTE_ADDR'], 'status' => $status];
