	...
	public function sendVoucher($voucher_id) {
		...
		if ($voucher_info) {
			...
			// If voucher does not belong to an order
				...
				$data['text_greeting'] = sprintf($this->language->get('text_greeting'), $this->currency->format($voucher_info['amount'], $this->config->get('config_currency'), $this->currency->getValue($this->config->get('config_currency'))));
