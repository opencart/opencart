...
class ControllerCommonDevelopment extends Controller {
	...
	public function index() {
		if ($this->config->get('config_development')) {
			...
			// Clear Log
			if ($user->hasPermission('modify', 'tool/error_log')) {
				$this->clearLog();
			}
			
			// Refresh Modification
			if ($user->hasPermission('modify', 'extension/modification')) {
			...
			foreach ($files as $file) {
				...
				foreach ($files as $file) {
					...
					if (substr($file, 0, 7) == 'catalog') {
						$path = DIR_APPLICATION . str_replace('../', '', substr($file, 8));
					}
						...
						if ($files) {
							foreach ($files as $file) {
								...
								if (substr($file, 0, strlen(DIR_APPLICATION)) == DIR_APPLICATION) {
									$key = 'catalog/' . substr($file, strlen(DIR_APPLICATION));
								}
