                                                                            ...
                                                                            if ($next_order_product) {
                                                                                ...
                                                                                if ($next_order_info) {
                                                                                    ...
                                                                                    if ($next_date_cycle >= 0 && $next_order_info['order_status_id'] == $this->config->get('config_subscription_pending_status_id')) {
                                                                                        ...
                                                                                        if ($product_subscription_info && (int)$product_subscription_info['cycle'] >= 0 && $product_subscription_info['duration'] == $next_subscription['duration']) {
                                                                                            if ($product_subscription_info['subscription_plan_id'] == $next_subscription['subscription_plan_id']) {
                                                                                                // Add Transaction
                                                                                                $this->model_account_subscription->addTransaction($subscription_id, $next_subscription['order_id'], $this->language->get('mail_text_promotion'), $next_subscription['amount'], $next_subscription['type'], $next_order_info['payment_method'], $next_order_info['payment_code']);
                                                                                            } else {
                                                                                                $this->model_checkout_subscription->addHistory($next_subscription['order_id'], $subscription_status_id, $this->language->get('mail_error_subscription_plan'));
                                                                                            }
                                                                                        }
                                                                                    } else {
