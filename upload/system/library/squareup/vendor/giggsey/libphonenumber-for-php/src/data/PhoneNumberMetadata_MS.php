...
class PhoneNumberMetadata_MS extends PhoneMetadata
{
    ...
    public function __construct()
    {
        $this->generalDesc = (new PhoneNumberDesc())
            ->setNationalNumberPattern('(?:[58]\d\d|664|900)\d{7}')
            ->setPossibleLengthLocalOnly([7])
            ->setPossibleLength([10]);
        $this->mobile = (new PhoneNumberDesc())
            ->setNationalNumberPattern('664(?:3(?:49|9[1-6])|49[2-6])\d{4}')
            ->setExampleNumber('6644923456')
            ->setPossibleLengthLocalOnly([7]);
        $this->premiumRate = (new PhoneNumberDesc())
            ->setNationalNumberPattern('900[2-9]\d{6}')
            ->setExampleNumber('9002123456');
        $this->fixedLine = (new PhoneNumberDesc())
            ->setNationalNumberPattern('6644(?:1[0-3]|91)\d{4}')
            ->setExampleNumber('6644912345')
            ->setPossibleLengthLocalOnly([7]);
