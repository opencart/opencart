...
class ControllerPaymentAuthorizeNetSim extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('payment/authorizenet_sim', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->ssl('payment/authorizenet_sim', 'token=' . $this->session->data['token'], true);
