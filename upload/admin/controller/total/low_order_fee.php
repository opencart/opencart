...
class ControllerTotalLowOrderFee extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('total/low_order_fee', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->ssl('total/low_order_fee', 'token=' . $this->session->data['token'], true);
