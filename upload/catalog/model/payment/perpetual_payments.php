...
class ModelPaymentPerpetualPayments extends Model {
  	public function getMethod($country_id, $zone_id) {
		...
		if ($this->config->get('perpetual_payments_status')) {
      		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('perpetual_payments_geo_zone_id') . "' AND country_id = '" . (int)$country_id . "' AND (zone_id = '" . (int)$zone_id . "' OR zone_id = '0')");
