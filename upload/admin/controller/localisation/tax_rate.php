...
class ControllerLocalisationTaxRate extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->ssl('localisation/tax_rate', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('localisation/tax_rate', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->ssl('localisation/tax_rate/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->ssl('localisation/tax_rate/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['tax_rates'][] = array(
				...
				'edit'          => $this->url->ssl('localisation/tax_rate/edit', 'token=' . $this->session->data['token'] . '&tax_rate_id=' . $result['tax_rate_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->ssl('localisation/tax_rate', 'token=' . $this->session->data['token'] . '&sort=tr.name' . $url, true);
		$data['sort_rate'] = $this->url->ssl('localisation/tax_rate', 'token=' . $this->session->data['token'] . '&sort=tr.rate' . $url, true);
		$data['sort_type'] = $this->url->ssl('localisation/tax_rate', 'token=' . $this->session->data['token'] . '&sort=tr.type' . $url, true);
		$data['sort_geo_zone'] = $this->url->ssl('localisation/tax_rate', 'token=' . $this->session->data['token'] . '&sort=gz.name' . $url, true);
		$data['sort_date_added'] = $this->url->ssl('localisation/tax_rate', 'token=' . $this->session->data['token'] . '&sort=tr.date_added' . $url, true);
		$data['sort_date_modified'] = $this->url->ssl('localisation/tax_rate', 'token=' . $this->session->data['token'] . '&sort=tr.date_modified' . $url, true);
		...
		$pagination->url = $this->url->ssl('localisation/tax_rate', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['tax_rate_id'])) {
			$data['action'] = $this->url->ssl('localisation/tax_rate/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->ssl('localisation/tax_rate/edit', 'token=' . $this->session->data['token'] . '&tax_rate_id=' . $this->request->get['tax_rate_id'] . $url, true);
		}

		$data['cancel'] = $this->url->ssl('localisation/tax_rate', 'token=' . $this->session->data['token'] . $url, true);
