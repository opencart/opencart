...
class Product extends \Opencart\System\Engine\Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			$product_data['rating'] = round($query->row['rating']);
	...
	public function getOnetimeSpecial($product_id) {
	...
	public function getReward($product_id) {
		$query = $this->db->query("SELECT points FROM " . DB_PREFIX . "product_reward WHERE product_id = '" . (int)$product_id . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "'");
		
		return $query->row;
	}
	
	public function getDownloads($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_download p2d LEFT JOIN " . DB_PREFIX . "download d ON (p2d.download_id = d.download_id) LEFT JOIN " . DB_PREFIX . "download_description dd ON (d.download_id = dd.download_id) WHERE p2d.product_id = '" . (int)$product_id . "' AND dd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getOptionsByProductOptionId($product_option_id, $product_id) {
		$product_option_data = [];
	...
	public function getOnetimeDiscount($product_id, $discount_quantity) {
		$query = $this->db->query("SELECT price FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . (int)$product_id . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND quantity <= '" . (int)$discount_quantity . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY quantity DESC, priority ASC, price ASC LIMIT 1");
	...
	public function getLayoutId($product_id) {
