...
class ModelCatalogCategory extends Model {
	...
	public function editCategory($category_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "category SET `parent_id` = '" .(isset($data['parent_id']) ? (int)$data['parent_id'] : 0)."', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
