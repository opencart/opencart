...
class Ssr extends \Opencart\System\Engine\Model {
	 ...
	 *
	 * $ssr_id = $this->model_setting_ssr->addSsr($ssr_data);
	 */
	public function addSsr(array $data): int {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "ssr` SET `code` = '" . $this->db->escape($data['code']) . "', `description` = '" . $this->db->escape($data['description']) . "', `action` = '" . $this->db->escape($data['action']) . "', `status` = '" . (bool)$data['status'] . "', `date_modified` = NOW()");
	 ...
	 * $this->load->model('setting/ssr');
	 *
	 * $this->model_setting_ssr->editSsr($ssr_id);
	 */
	...
	public function getSsrs(array $data = []): array {
		$sql = "SELECT * FROM `" . DB_PREFIX . "ssr` ORDER BY `code` ASC";
