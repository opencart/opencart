...
<table class="table table-striped table-bordered">
  ...
  <tr>
    ...
    <td id="capture_status">
      <?php if ($firstdata_order['capture_status'] == 1) { ?>
      <span class="capture_text"><?php echo $text_yes; ?></span><?php } else { ?>
      <span class="capture_text"><?php echo $text_no; ?></span>&nbsp;&nbsp;<?php if ($firstdata_order['void_status'] == 0) { ?>
      <a class="button btn btn-primary" id="btn_capture"><?php echo $btn_capture; ?></a>
      <span class="btn btn-primary" id="img_loading_capture" style="display:none;"><i class="fa fa-cog fa-spin fa-lg"></i></span><?php } ?><?php } ?>
    </td>
  </tr>
  <tr>
    <td><?php echo $text_void_status; ?></td>
    <td id="void_status">
      <?php if ($firstdata_order['void_status'] == 1) { ?>
      <span class="void_text"><?php echo $text_yes; ?></span><?php } else { ?>
...
<form method="POST" id="voidform" action="<?php echo $action_url; ?>">
  ...
  <input type="hidden" name="txntype" value="void"/> <input type="hidden" name="timezone" value="GMT"/>
...
<form method="POST" id="captureform" action="<?php echo $action_url; ?>">
  ...
  <input type="hidden" name="transactionNotificationURL" value="<?php echo $notify_url; ?>"/>
  <input type="hidden" name="txntype" value="postauth"/> <input type="hidden" name="timezone" value="GMT"/>
  <input type="hidden" name="txndatetime" value="<?php echo $request_timestamp; ?>"/>
  <input type="hidden" name="hash" value="<?php echo $hash; ?>"/>
  <input type="hidden" name="storename" value="<?php echo $merchant_id; ?>"/>
  <input type="hidden" name="chargetotal" value="<?php echo $amount; ?>"/>
