<?php 
class ControllerCustomerSendMail extends Controller {
	private $error = array();
	 
	public function index() {
		$this->load->language('customer/send_mail');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			$this->redirect($this->url->https('customer/send_mail'));
		}
		...
		$this->data['error_message'] = @$this->error['message'];
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => $this->url->https('customer/send_mail'),
		...
		$this->data['action'] = $this->url->https('customer/send_mail');
    	$this->data['cancel'] = $this->url->https('customer/send_mail');
		...
		$this->id       = 'content';
		$this->template = 'customer/send_mail.tpl';
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'customer/send_mail')) {
