...
class ModelReportDashboard extends Model {
	...
	public function getTotalMarketingsByDay() {
		...
		$query = $this->db->query("SELECT SUM(m.clicked) AS click, (SELECT COUNT(o.order_id) FROM `" . DB_PREFIX . "order` o WHERE o.order_status_id = '" . (int)$this->config->get('config_complete_status_id') . "' AND o.tracking = m.code AND DATE(o.date_added) = DATE(m.date_added) AND HOUR(o.date_added) = HOUR(m.date_added)) AS sale, HOUR(m.date_added) AS hour FROM `" . DB_PREFIX . "marketing` m WHERE DATE(m.date_added) = DATE(NOW()) GROUP BY HOUR(m.date_added) ORDER BY m.date_added ASC");
	...
	public function getTotalMarketingsByMonth() {
		...
		$query = $this->db->query("SELECT SUM(m.clicked) AS click, (SELECT COUNT(o.order_id) FROM `" . DB_PREFIX . "order` o WHERE o.order_status_id = '" . (int)$this->config->get('config_complete_status_id') . "' AND o.tracking = m.code AND DATE(o.date_added) = DATE(m.date_added)) AS sale, m.date_added FROM `" . DB_PREFIX . "marketing` m WHERE DATE(m.date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' GROUP BY DATE(m.date_added)");
	...
	public function getTotalMarketingsByYear() {
		...
		$query = $this->db->query("SELECT SUM(m.clicked) AS click, (SELECT COUNT(o.order_id) FROM `" . DB_PREFIX . "order` o WHERE o.order_status_id = '" . (int)$this->config->get('config_complete_status_id') . "' AND o.tracking = m.code AND MONTH(o.date_added) = MONTH(m.date_added) AND YEAR(o.date_added) = YEAR(m.date_added)) AS sale, m.date_added FROM `" . DB_PREFIX . "marketing` m WHERE YEAR(date_added) = YEAR(NOW()) GROUP BY MONTH(date_added)");
