...
<div class="box">
  ...
  <div class="content">
    <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <tbody id="module-row<?php echo $module_row; ?>">
          <tr>
            ...
            <td class="left"><select name="manufacturer_<?php echo $module_row; ?>_axis">
                ...
                <?php } ?>
                <?php if (${'manufacturer_' . $module . '_axis'} == 'vertical') { ?>
                <option value="vertical" selected="selected"><?php echo $text_vertical; ?></option>
                <?php } else { ?>
                <option value="vertical"><?php echo $text_vertical; ?></option>
                <?php } ?>
              </select></td>
            <td class="left"><select name="manufacturer_<?php echo $module_row; ?>_layout_id">
                <?php foreach ($layouts as $layout) { ?>
                <?php if ($layout['layout_id'] == ${'manufacturer_' . $module . '_layout_id'}) { ?>
            ...
            <td class="left"><select name="manufacturer_<?php echo $module_row; ?>_position">
                <?php if (${'manufacturer_' . $module . '_position'} == 'content_top') { ?>
                <option value="content_top" selected="selected"><?php echo $text_content_top; ?></option>
                <?php } else { ?>
                <option value="content_top"><?php echo $text_content_top; ?></option>
                <?php } ?>
                <?php if (${'manufacturer_' . $module . '_position'} == 'content_bottom') { ?>
                <option value="content_bottom" selected="selected"><?php echo $text_content_bottom; ?></option>
                <?php } else { ?>
                <option value="content_bottom"><?php echo $text_content_bottom; ?></option>
                <?php } ?>
                <?php if (${'manufacturer_' . $module . '_position'} == 'column_left') { ?>
                <option value="column_left" selected="selected"><?php echo $text_column_left; ?></option>
                <?php } else { ?>
                <option value="column_left"><?php echo $text_column_left; ?></option>
                <?php } ?>
                <?php if (${'manufacturer_' . $module . '_position'} == 'column_right') { ?>
                <option value="column_right" selected="selected"><?php echo $text_column_right; ?></option>
                <?php } else { ?>
                <option value="column_right"><?php echo $text_column_right; ?></option>
                <?php } ?>
              </select></td>
            <td class="left"><select name="manufacturer_<?php echo $module_row; ?>_status">
                <?php if (${'manufacturer_' . $module . '_status'}) { ?>
            ...
            <td class="right"><input type="text" name="manufacturer_<?php echo $module_row; ?>_sort_order" value="<?php echo ${'manufacturer_' . $module . '_sort_order'}; ?>" size="3" /></td>
      ...
      <input type="hidden" name="manufacturer_module" value="<?php echo $manufacturer_module; ?>" />
    </form>
  </div>
</div>
...
function addModule() {	
	...
	html += '  <tr>';
	html += '    <td class="left"><input type="text" name="manufacturer_' + module_row + '_limit" value="5" size="1" /></td>';
	html += '    <td class="left"><input type="text" name="manufacturer_' + module_row + '_scroll" value="3" size="1" /></td>';
	html += '    <td class="left"><input type="text" name="manufacturer_' + module_row + '_width" value="450" size="3" /> <input type="text" name="manufacturer_' + module_row + '_height" value="450" size="3" /></td>';	
	html += '    <td class="left"><input type="text" name="manufacturer_' + module_row + '_image_width" value="80" size="3" /> <input type="text" name="manufacturer_' + module_row + '_image_height" value="80" size="3" /></td>';	
	html += '    <td class="left"><select name="manufacturer_' + module_row + '_axis">';
	html += '    	<option value="horizontal"><?php echo $text_horizontal; ?></option>';
	html += '    	<option value="vertical"><?php echo $text_vertical; ?></option>';
	html += '    </select></td>';	
	html += '    <td class="left"><select name="manufacturer_' + module_row + '_layout_id">';
	<?php foreach ($layouts as $layout) { ?>
	html += '      <option value="<?php echo $layout['layout_id']; ?>"><?php echo $layout['name']; ?></option>';
	<?php } ?>
	html += '    </select></td>';
	html += '    <td class="left"><select name="manufacturer_' + module_row + '_position">';
	html += '      <option value="content_top"><?php echo $text_content_top; ?></option>';
	html += '      <option value="content_bottom"><?php echo $text_content_bottom; ?></option>';
	html += '      <option value="column_left"><?php echo $text_column_left; ?></option>';
	html += '      <option value="column_right"><?php echo $text_column_right; ?></option>';
	html += '    </select></td>';
	html += '    <td class="left"><select name="manufacturer_' + module_row + '_status">';
