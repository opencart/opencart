...
class Marketing extends \Opencart\System\Engine\Model {
	...
	public function getMarketings($data = []) {
		...
		$sql = "SELECT *, (SELECT COUNT(*) FROM `" . DB_PREFIX . "order` `o` LEFT JOIN `" . DB_PREFIX . "order_customer_affiliate` oca ON (`o`.`order_id` = oca.`order_id`) WHERE (" . implode(" OR ", $implode) . ") AND oca.`marketing_id` = m.`marketing_id`) AS orders FROM " . DB_PREFIX . "marketing m";
