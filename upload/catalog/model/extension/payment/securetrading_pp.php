...
class ModelExtensionPaymentSecureTradingPp extends Model {
	...
	public function confirmOrder($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "securetrading_pp_order` SET `settle_type` = '" . $this->db->esape($this->config->get('payment_securetrading_pp_settle_status')) . "', `modified` = NOW(), `currency_code` = '" . $this->db->escape($order_info['currency_code']) . "', `total` = '" . (float)$amount . "' WHERE order_id = '" . (int)$order_info['order_id'] . "'");
