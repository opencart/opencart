...
class ModelExtensionOpenBayEbayProfile extends Model{
	...
	public function get($id) {
		...
		if ($qry->num_rows) {
			$row                = $qry->row;
			$row['link_edit']   = $this->url->link('extension/openbay/ebay_profile/edit', 'user_token=' . $this->session->data['user_token'] . '&ebay_profile_id=' . $row['ebay_profile_id']);
			$row['link_delete'] = $this->url->link('extension/openbay/ebay_profile/delete', 'user_token=' . $this->session->data['user_token'] . '&ebay_profile_id=' . $row['ebay_profile_id']);
