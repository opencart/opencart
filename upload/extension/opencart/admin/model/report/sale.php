...
class Sale extends \Opencart\System\Engine\Model {
	...
	public function getOrders(array $data = []): array {
		$sql = "SELECT MIN(o.`date_added`) AS date_start, MAX(o.`date_added`) AS date_end, COUNT(*) AS orders, SUM((SELECT SUM(op.`quantity`) FROM `" . DB_PREFIX . "order_product` op WHERE op.`order_id` = o.`order_id` GROUP BY op.`order_id`)) AS products, SUM((SELECT SUM(op.`quantity`) FROM (`" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "subscription` s) INNER JOIN `" . DB_PREFIX . "subscription_transaction` st ON (s.`subscription_id` = st.`subscription_id`) WHERE op.`order_id` = o.`order_id` AND s.`order_id` = op.`order_id` AND s.`customer_id` = o.`customer_id` AND s.`order_product_id` = op.`order_product_id` AND st.`order_id` = s.`order_id` AND st.`amount` > '0' AND st.`payment_method` = o.`payment_method` AND st.`payment_code` = o.`payment_code` GROUP BY op.`order_id`)) AS subscriptions, SUM((SELECT SUM(ot.`value`) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.`order_id` = o.`order_id` AND ot.`code` = 'tax' GROUP BY ot.`order_id`)) AS tax, SUM(o.`total`) AS `total` FROM `" . DB_PREFIX . "order` o";
