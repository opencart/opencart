...
class Tag extends \Opencart\System\Engine\Controller {
	 ...
	 * Add Tag
	 *
	 * Adds task to generate new tag data.
	 *
	 * Trigger admin/model/catalog/tag.addTag/after
	 *
	 * @param string                $route
	 ...
	 * @return void
	 */
	public function addTag(string &$route, array &$args, &$output): void {
		...
		$tag_ids = $this->model_catalog_tag->getTags((int)$args['tag_id']);

		foreach ($store_ids as $store_id) {
			$task_data = [
				'code'   => 'tag.info.' . $store_id . '.' . $output,
				'action' => 'task/catalog/tag.info',
				'args'   => [
					'tag_id'   => $output,
					'store_id' => $store_id
				]
