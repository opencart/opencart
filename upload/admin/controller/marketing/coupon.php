...
class ControllerMarketingCoupon extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->ssl('marketing/coupon', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('marketing/coupon', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->ssl('marketing/coupon/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->ssl('marketing/coupon/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['coupons'][] = array(
				...
				'edit'       => $this->url->ssl('marketing/coupon/edit', 'token=' . $this->session->data['token'] . '&coupon_id=' . $result['coupon_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->ssl('marketing/coupon', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_code'] = $this->url->ssl('marketing/coupon', 'token=' . $this->session->data['token'] . '&sort=code' . $url, true);
		$data['sort_discount'] = $this->url->ssl('marketing/coupon', 'token=' . $this->session->data['token'] . '&sort=discount' . $url, true);
		$data['sort_date_start'] = $this->url->ssl('marketing/coupon', 'token=' . $this->session->data['token'] . '&sort=date_start' . $url, true);
		$data['sort_date_end'] = $this->url->ssl('marketing/coupon', 'token=' . $this->session->data['token'] . '&sort=date_end' . $url, true);
		$data['sort_status'] = $this->url->ssl('marketing/coupon', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);
		...
		$pagination->url = $this->url->ssl('marketing/coupon', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['coupon_id'])) {
			$data['action'] = $this->url->ssl('marketing/coupon/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->ssl('marketing/coupon/edit', 'token=' . $this->session->data['token'] . '&coupon_id=' . $this->request->get['coupon_id'] . $url, true);
		}

		$data['cancel'] = $this->url->ssl('marketing/coupon', 'token=' . $this->session->data['token'] . $url, true);
	...
	public function history() {
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->ssl('marketing/coupon/history', 'token=' . $this->session->data['token'] . '&coupon_id=' . $this->request->get['coupon_id'] . '&page={page}', true);
