...
class ModelReportDashboard extends Model {
	...
	public function getTotalMarketingsByYear() {
		...
		$query = $this->db->query("SELECT SUM(m.clicked) AS click, (SELECT COUNT(o.order_id) FROM `" . DB_PREFIX . "order` o WHERE o.order_status_id = '" . (int)$this->config->get('config_complete_status_id') . "' AND o.tracking = m.code AND MONTH(o.date_added) = MONTH(m.date_added) AND YEAR(o.date_added) = YEAR(m.date_added)) AS `order`, m.date_added FROM `" . DB_PREFIX . "marketing` m WHERE YEAR(date_added) = YEAR(NOW()) GROUP BY MONTH(date_added)");
