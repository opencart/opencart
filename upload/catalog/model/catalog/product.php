...
class Product extends \Opencart\System\Engine\Model {
	...
	public function getTotalProducts(array $data = []): int {
		...
		$sql .= " LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`product_id` = pd.`product_id` AND pd.`language_id` = '" . (int)$this->config->get('config_language_id') . "') LEFT JOIN `" . DB_PREFIX . "product_to_store` p2s ON (p.`product_id` = p2s.`product_id` AND p2s.`store_id` = '" . (int)$this->config->get('config_store_id') . "') WHERE 1=1";
