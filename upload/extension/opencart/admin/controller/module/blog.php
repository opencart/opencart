	...
	public function index(): void {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = [
				...
				'href' => $this->url->link('extension/opencart/module/blog')
			];
		} else {
			$data['breadcrumbs'][] = [
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/opencart/module/blog', 'module_id=' . $this->request->get['module_id'])
			];
		}

		if (!isset($this->request->get['module_id'])) {
			$data['save'] = $this->url->link('extension/opencart/module/blog.save');
		} else {
			$data['save'] = $this->url->link('extension/opencart/module/blog.save', 'module_id=' . $this->request->get['module_id']);
		}
	...
	public function save(): void {
		...
		if (!$json) {
			...
			if (!$this->request->post['module_id']) {
				$json['module_id'] = $this->model_setting_module->addModule('opencart.blog', $this->request->post);
			} else {
