...
class ModelSaleAffiliate extends Model {
	...
	public function editAffiliate($affiliate_id, $data) {
		...
		if ($data['password']) {
			$this->db->query("UPDATE " . DB_PREFIX . "affiliate SET salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . sha1($salt . sha1($salt . sha1($data['password']))) . "' WHERE affiliate_id = '" . (int)$affiliate_id . "'");
		}
	}
