...
class Language extends \Opencart\System\Engine\Model {
	...
	public function editLanguage($language_id, $data) {
		...
		if ($language_query->row['code'] != $data['code']) {
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "settin`g SET `value` = '" . $this->db->escape((string)$data['code']) . "' WHERE `key` = 'config_admin_language' AND `value` = '" . $this->db->escape($language_query->row['code']) . "'");
		}
