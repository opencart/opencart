...
class ControllerPaymentAmazonLoginPay extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('payment/amazon_login_pay', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->ssl('payment/amazon_login_pay', 'token=' . $this->session->data['token'], true);
