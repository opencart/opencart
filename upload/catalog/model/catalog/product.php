...
class Product extends \Opencart\System\Engine\Model {
	...
	public function getProducts(array $data = []): array {
		...
		if (!empty($data['filter_category_id'])) {
			...
			$sql .= " FROM `" . DB_PREFIX . "product_to_store` `p2s` LEFT JOIN `" . DB_PREFIX . "product` `p` ON (`p`.`product_id` = `p2s`.`product_id` AND `p`.`status` = '1' AND AND `p2s`.`store_id` = '" . (int)$this->config->get('config_store_id') . "' `p`.`date_available` <= NOW())";
		}
