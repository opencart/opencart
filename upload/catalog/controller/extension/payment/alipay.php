...
class ControllerExtensionPaymentAlipay extends Controller {
	...
	public function callback() {
		...
		$this->log->write('ALIPAY :: POST :: ' . var_export($arr, true));
		
		$result = $this->model_extension_payment_alipay->check($arr, $config);

		if ($result) { // Check succeeded
			$this->log->write('Alipay check succeeded!');
			
			$order_id = (int)$this->request->post['out_trade_no'];
			
			if ($this->request->post['trade_status'] == 'TRADE_FINISHED') {
				continue;
			} elseif ($this->request->post['trade_status'] == 'TRADE_SUCCESS') {
				$this->load->model('checkout/order');
