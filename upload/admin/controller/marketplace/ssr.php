...
class Ssr extends \Opencart\System\Engine\Controller {
	...
	public function getList(): string {
		...
		foreach ($results as $result) {
			$data['ssrs'][] = ['run' => $this->url->link('marketplace/ssr.run', 'user_token=' . $this->session->data['user_token'] . '&ssr_id=' . $result['ssr_id'])] + $result;
		}

		$ssr_total = $this->model_setting_ssr->getTotalSsrs();

		// Pagination
		$data['total'] = $ssr_total;
		$data['page'] = $page;
		$data['limit'] = $this->config->get('config_pagination_admin');
		$data['pagination'] = $this->url->link('marketplace/ssr.list', 'user_token=' . $this->session->data['user_token'] . '&page={page}');
