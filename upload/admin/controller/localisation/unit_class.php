...
class ControllerLocalisationUnitClass extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$this->data['unit_classes'][] = array(
				...
				'action'          => $action,
				'product_total'   => $this->model_catalog_product->getTotalProductsByUnitClassId($result['unit_class_id'])
			);
		}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['unit_class_description'])) {
			...
			$this->data['unit_class_description'] = $this->model_localisation_unit_class->getUnitClassDescriptions($this->request->get['unit_class_id']);
		} else {
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['unit_class_description'] as $language_id => $value) {
			...
			if (!$value['unit'] || (utf8_strlen($value['unit']) > 32)) {
