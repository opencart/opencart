...
$(document).ready(function() {
		...
		$('#continue-button').click(function() {
			...
			if ($('input[name="addressSelected"]').val() == '0') {
				$('#addressBookWidgetDiv').before('<div class="alert alert-danger"><i class="mi mi-exclamation-circle">error</i><?php echo $error_shipping_address; ?></div>');
			} else if ($('input[name="shipping_method"]:checked').length == 0) {
				$('#addressBookWidgetDiv').before('<div class="alert alert-danger"><i class="mi mi-exclamation-circle">error</i><?php echo $error_shipping; ?></div>');
			} else {
				$.ajax({
		...
		new OffAmazonPayments.Widgets.AddressBook({
			...
			onAddressSelect: function(orderReference) {
				...
				$.get('<?php echo $shipping_quotes; ?>&AmazonOrderReferenceId=' + AmazonOrderReferenceId, {}, function(data) {
					...
					if (data.error) {
						$('#addressBookWidgetDiv').before('<div class="alert alert-danger"><i class="mi mi-exclamation-circle">error</i>' + data.error + '</div>');
					} else if (data.quotes) {
						var html = '';
