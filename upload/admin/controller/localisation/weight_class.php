...
class ControllerLocalisationWeightClass extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->ssl('localisation/weight_class', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('localisation/weight_class', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->ssl('localisation/weight_class/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->ssl('localisation/weight_class/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['weight_classes'][] = array(
				...
				'unit'            => $result['unit'],
				'value'           => $result['value'],
				'edit'            => $this->url->ssl('localisation/weight_class/edit', 'token=' . $this->session->data['token'] . '&weight_class_id=' . $result['weight_class_id'] . $url, true)
			);
		}
		...
		$data['sort_title'] = $this->url->ssl('localisation/weight_class', 'token=' . $this->session->data['token'] . '&sort=title' . $url, true);
		$data['sort_unit'] = $this->url->ssl('localisation/weight_class', 'token=' . $this->session->data['token'] . '&sort=unit' . $url, true);
		$data['sort_value'] = $this->url->ssl('localisation/weight_class', 'token=' . $this->session->data['token'] . '&sort=value' . $url, true);
		...
		$pagination->url = $this->url->ssl('localisation/weight_class', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['weight_class_id'])) {
			$data['action'] = $this->url->ssl('localisation/weight_class/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->ssl('localisation/weight_class/edit', 'token=' . $this->session->data['token'] . '&weight_class_id=' . $this->request->get['weight_class_id'] . $url, true);
		}

		$data['cancel'] = $this->url->ssl('localisation/weight_class', 'token=' . $this->session->data['token'] . $url, true);
