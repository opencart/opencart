...
class ControllerAccountReturn extends Controller { 
	...
	public function index() {
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($return_total) ? (($page - 1) * $this->config->get('config_catalog_limit')) + 1 : 0, ((($page - 1) * $this->config->get('config_catalog_limit')) > ($return_total - $this->config->get('config_catalog_limit'))) ? $return_total : ((($page - 1) * $this->config->get('config_catalog_limit')) + $this->config->get('config_catalog_limit')), $return_total, ceil($return_total / $this->config->get('config_catalog_limit')));
	...
	public function insert() {
		...
		if ($this->config->get('config_return_id')) {
			...
			if ($information_info) {
				$data['text_agree'] = sprintf($this->language->get('text_agree'), $this->url->link('information/information/info', 'information_id=' . $this->config->get('config_return_id'), 'SSL'), $information_info['title'], $information_info['title']);
			} else {
				$data['text_agree'] = '';
			}
		} else {
