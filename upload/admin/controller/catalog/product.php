...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		$filter_data = array_merge($this->provider->getParams(), array('start' => ($page - 1) * $this->config->get('config_limit_admin'), 'limit' => $this->config->get('config_limit_admin')));
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'edit'       => $this->provider->link('catalog/product/edit', ['product_id' => $result['product_id'], 'master_id' => ($result['master_id']) ? $result['master_id'] : '']),
				'variant'    => (!$result['master_id'] ? $this->provider->link('catalog/product/add', ['master_id' => $result['master_id']]) : ''),
				'compare'    => ($result['master_id'] ? $this->provider->link('catalog/product_compare', ['product_id' => $result['product_id'], 'master_id' => $result['master_id']]) : ''),
		...
		$order = ($filter_data['order'] === 'ASC') ? 'DESC' : 'ASC';

		foreach ($sorts as $key => $sort) {
			$data[$key] = $this->provider->link('catalog/product', ['sort' => $sort, 'order' => $order]);
		}
		...
		$data['pagination'] = $this->load->controller('common/pagination', array(
			...
			'url'   => $this->provider->link('', ['page' => '{page}'])
		));
	...
	protected function getForm() {
		...
		$text_form = $this->provider->hasRequest('product_id') ? $this->language->get('text_edit') : $this->language->get('text_add');
		...
		$this->breadcrumbs->set($text_form);
		...
		} elseif ($this->provider->hasRequest('master_id')) {
		...
		if ($this->provider->hasRequest('master_id')) {
				...
				foreach ($language as $language_id => $keyword) {
					if ($keyword) {
						...
						foreach ($seo_urls as $seo_url) {
							if (($seo_url['store_id'] == $store_id) && ($seo_url['language_id'] == $language_id) && (!$this->provider->hasRequest('product_id') || (($seo_url['query'] != 'product_id=' . $this->request->get['product_id'])))) {
...
*/			$data['name'] = strip_tags(html_entity_decode($product_info['name'], ENT_QUOTES, 'UTF-8'));
			...
			$this->response->setOutput($this->load->view('catalog/product_master', $data));
		}
	}
	...
	public function autocomplete() {
		...
		if ($this->provider->hasRequest('filter_name') || $this->provider->hasRequest('filter_model')) {
			$this->load->model('catalog/product');
			$this->load->model('catalog/option');

			if ($this->provider->hasRequest('filter_name')) {
				$filter_name = $this->request->get['filter_name'];
			} else {
				$filter_name = '';
			}

			if ($this->provider->hasRequest('filter_model')) {
				$filter_model = $this->request->get['filter_model'];
			} else {
				$filter_model = '';
			}

			if ($this->provider->hasRequest('limit')) {
