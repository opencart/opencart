		...
		if ($status) {
			...
			if ($this->config->get('royal_mail_special_delivery_500_status') && $address['iso_code_2'] == 'GB') {
				...
				if ((float)$cost) {
					...
					if ($this->config->get('royal_mail_display_insurance')) {
						$title .= ' (' . $this->language->get('text_insurance') . ' ' . $this->currency->format(500, $this->request->cookie['currency']) . ')';
					}
					...
					$quote_data['special_delivery_500'] = array(
						...
						'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('royal_mail_tax_class_id'), $this->config->get('config_tax')), $this->request->cookie['currency'])
					);
				}
			}
			...
			if ($this->config->get('royal_mail_special_delivery_1000_status') && $address['iso_code_2'] == 'GB') {
				...
				if ((float)$cost) {
					...
					if ($this->config->get('royal_mail_display_insurance')) {
						$title .= ' (' . $this->language->get('text_insurance') . ' ' . $this->currency->format(1000, $this->request->cookie['currency']) . ')';
			...
			if ($this->config->get('royal_mail_special_delivery_2500_status') && $address['iso_code_2'] == 'GB') {
				...
				if ((float)$cost) {
					...
					if ($this->config->get('royal_mail_display_insurance')) {
						$title .= ' (' . $this->language->get('text_insurance') . ' ' . $this->currency->format(2500, $this->request->cookie['currency']) . ')';
					}
