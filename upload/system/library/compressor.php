...
class Compressor {
	 ...
	 *
	 * Returns aggregated compression statistics for the last compression
	 * operation performed by the specified algorithm.
	 *
	 * Provided metrics:
	 *  - algorithm: algorithm key used (string)
	 *  - level: compression level used in last operation
	 *  - original_size: original (uncompressed) size in bytes
	 *  - compressed_size: compressed size in bytes
	 *  - ratio: compressed_size / original_size (lower is better)
	 *  - efficiency: 1 - ratio (fraction of size saved, 0..1)
	 *  - percent: ratio * 100 (compressed size as percent of original)
	 *
	 * If the algorithm has not yet performed a compression or is unavailable
	 * the numeric metrics default to 0.0.
	 *
	 * @param string $algorithm the compression algorithm (gzip, brotli, zstd)
	 *
	 * @return array<string, float|int|string> associative stats array
	 *                                         [
	 *                                         'algorithm' => string,
	 *                                         'level' => int,
	 *                                         'original_size' => float,
	 ...
	 * @example
	 *
	 * $stats = $this->compressor->getStats('gzip');
	 */
	public function getStats(string $algorithm): array {
		$adapter = $this->load->library('compressor/' . $algorithm);

		$original = method_exists($adapter, 'getOriginalSize') ? $adapter->getOriginalSize() : 0;
		$compressed = method_exists($adapter, 'getCompressedSize') ? $adapter->getCompressedSize() : 0;
		$level = method_exists($adapter, 'getLastLevel') ? $adapter->getLastLevel() : 0;
		$ratio = $original > 0 ? ($compressed / $original) : 0.0;
		$efficiency = $ratio > 0 ? max(0.0, min(1.0, 1 - $ratio)) : 0.0;
		$percent = $ratio * 100;

		return [
			'algorithm'       => $algorithm,
			'level'           => $level,
			'original_size'   => (float)$original,
			'compressed_size' => (float)$compressed,
			'ratio'           => $ratio,
	...
	/**
	 * Reset compression statistics for the given algorithm.
	 *
	 * @param string $algorithm Algorithm key (gzip, brotli, zstd)
	 *
	 * @return bool true on success (adapter found & reset), false otherwise
	 *
	 * @example
	 * $this->compressor->resetStats('gzip');
	 */
	public function resetStats(string $algorithm): bool {
		$adapter = $this->load->library('compressor/' . $algorithm);

		if ($adapter && method_exists($adapter, 'reset')) {
			$adapter->reset();
