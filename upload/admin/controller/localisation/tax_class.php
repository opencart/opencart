...
class ControllerLocalisationTaxClass extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->ssl('localisation/tax_class', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('localisation/tax_class', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->ssl('localisation/tax_class/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->ssl('localisation/tax_class/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['tax_classes'][] = array(
				...
				'title'        => $result['title'],
				'edit'         => $this->url->ssl('localisation/tax_class/edit', 'token=' . $this->session->data['token'] . '&tax_class_id=' . $result['tax_class_id'] . $url, true)
			);
		}
		...
		$data['sort_title'] = $this->url->ssl('localisation/tax_class', 'token=' . $this->session->data['token'] . '&sort=title' . $url, true);
		...
		$pagination->url = $this->url->ssl('localisation/tax_class', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['tax_class_id'])) {
			$data['action'] = $this->url->ssl('localisation/tax_class/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->ssl('localisation/tax_class/edit', 'token=' . $this->session->data['token'] . '&tax_class_id=' . $this->request->get['tax_class_id'] . $url, true);
		}

		$data['cancel'] = $this->url->ssl('localisation/tax_class', 'token=' . $this->session->data['token'] . $url, true);
