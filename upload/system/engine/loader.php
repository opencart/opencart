...
class Loader {
			...
			if (class_exists($class)) {
				...
				foreach (get_class_methods($class) as $method) {
					if ((substr($method, 0, 2) != '__') && is_callable($class, $method))  {
						$proxy->{$method} = $this->modelCallback($route . '/' . $method);
					}
				}
	...
	public function library(string $route, array $args = []): object {
		...
		if (class_exists($class)) {
			...
			foreach (get_class_methods($class) as $method) {
				if ((substr($method, 0, 2) != '__') && is_callable($class, $method))  {
					$proxy->{$method} = $this->libraryCallback($route . '/' . $method, $args);
				}
			}

			$this->registry->set(str_replace('/', '_', $route), $proxy);
		} else {
	 ...
	 * @return	closure
	 */
	protected function modelCallback(string $route): callable {
	...
	protected function libraryCallback(string $route, array $parameters = []): callable {
		return function (mixed &...$args) use ($route, $parameters) {
			...
			if ($result) {
				...
				$callable = [$library, $method];

				if (is_callable($callable)) {
					$output = call_user_func_array($callable, $args);
				} else {
					throw new \Exception('Error: Could not call library/' . $route . '!');
				}
			}

			// Trigger the post events
			$result = $this->event->trigger('library/' . $trigger . '/after', [&$route, &$args, &$output]);
