...
class ControllerUserUserPermission extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->ssl('user/user_permission', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('user/user_permission', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->ssl('user/user_permission/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->ssl('user/user_permission/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['user_groups'][] = array(
				...
				'name'          => $result['name'],
				'edit'          => $this->url->ssl('user/user_permission/edit', 'token=' . $this->session->data['token'] . '&user_group_id=' . $result['user_group_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->ssl('user/user_permission', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		...
		$pagination->url = $this->url->ssl('user/user_permission', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['user_group_id'])) {
			$data['action'] = $this->url->ssl('user/user_permission/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->ssl('user/user_permission/edit', 'token=' . $this->session->data['token'] . '&user_group_id=' . $this->request->get['user_group_id'] . $url, true);
		}

		$data['cancel'] = $this->url->ssl('user/user_permission', 'token=' . $this->session->data['token'] . $url, true);
