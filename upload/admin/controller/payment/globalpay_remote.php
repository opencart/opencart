...
class ControllerPaymentGlobalpayRemote extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('payment/globalpay_remote', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->ssl('payment/globalpay_remote', 'token=' . $this->session->data['token'], true);
