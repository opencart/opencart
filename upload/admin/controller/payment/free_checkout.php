...
class ControllerPaymentFreeCheckout extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('payment/free_checkout', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->ssl('payment/free_checkout', 'token=' . $this->session->data['token'], true);
