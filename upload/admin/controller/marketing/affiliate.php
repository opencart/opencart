		...
		if ($customer_info) {
			...
			foreach ($custom_fields as $custom_field) {
				if ($custom_field['status']) {
					if (empty($this->request->post['custom_field'][$custom_field['custom_field_id']]) && $custom_field['location'] == 'affiliate' && $custom_field['required']) {
						$json['error']['custom_field_' . $custom_field['custom_field_id']] = sprintf($this->language->get('error_custom_field'), $custom_field['name']);
					} elseif ((!empty($custom_field['validation']) && !preg_match(html_entity_decode($custom_field['validation'], ENT_QUOTES, 'UTF-8'), $this->request->post['custom_field'][$custom_field['custom_field_id']]) && $custom_field['location'] == 'affiliate') && ($custom_field['type'] == 'text')) {
