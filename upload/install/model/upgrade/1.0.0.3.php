...
class ModelUpgrade1003 extends Model {
	public function upgrade() {
		...
		$this->db->query("ALTER TABLE `" . DB_PREFIX . "address` CHANGE `company` `company` VARCHAR(32) NOT NULL");
		...
		if (!$query->num_rows) {
			$this->db->query("ALTER TABLE `" . DB_PREFIX . "customer` ADD `custom_field` TEXT NOT NULL AFTER `zone_id`");
		}
		
		// Check order for missing custom field
