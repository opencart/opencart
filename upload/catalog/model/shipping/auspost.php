				...
				if ($response) {
					...
					if ($response_info['err_msg'] != 'OK') {
						...
						$quote_data['standard'] = array(
							...
							'text'         => $this->currency->format($this->tax->calculate($this->currency->convert($response_info['charge'], 'AUD', $this->request->cookie['currency']), $this->config->get('auspost_tax_class_id'), $this->config->get('config_tax')), $this->request->cookie['currency'], 1.0000000)
						);
					}
				}
			}
