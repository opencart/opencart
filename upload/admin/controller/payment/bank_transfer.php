...
class ControllerPaymentBankTransfer extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('payment/bank_transfer', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->ssl('payment/bank_transfer', 'token=' . $this->session->data['token'], true);
