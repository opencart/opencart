...
class ControllerCatalogDownload extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->ssl('catalog/download', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('catalog/download', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->ssl('catalog/download/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->ssl('catalog/download/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['downloads'][] = array(
				...
				'edit'        => $this->url->ssl('catalog/download/edit', 'token=' . $this->session->data['token'] . '&download_id=' . $result['download_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->ssl('catalog/download', 'token=' . $this->session->data['token'] . '&sort=dd.name' . $url, true);
		$data['sort_date_added'] = $this->url->ssl('catalog/download', 'token=' . $this->session->data['token'] . '&sort=d.date_added' . $url, true);
		...
		$pagination->url = $this->url->ssl('catalog/download', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['download_id'])) {
			$data['action'] = $this->url->ssl('catalog/download/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->ssl('catalog/download/edit', 'token=' . $this->session->data['token'] . '&download_id=' . $this->request->get['download_id'] . $url, true);
		}

		$data['cancel'] = $this->url->ssl('catalog/download', 'token=' . $this->session->data['token'] . $url, true);
