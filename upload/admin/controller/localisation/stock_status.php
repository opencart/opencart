...
class ControllerLocalisationStockStatus extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->ssl('localisation/stock_status', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('localisation/stock_status', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->ssl('localisation/stock_status/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->ssl('localisation/stock_status/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['stock_statuses'][] = array(
				...
				'name'            => $result['name'],
				'edit'            => $this->url->ssl('localisation/stock_status/edit', 'token=' . $this->session->data['token'] . '&stock_status_id=' . $result['stock_status_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->ssl('localisation/stock_status', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		...
		$pagination->url = $this->url->ssl('localisation/stock_status', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['stock_status_id'])) {
			$data['action'] = $this->url->ssl('localisation/stock_status/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->ssl('localisation/stock_status/edit', 'token=' . $this->session->data['token'] . '&stock_status_id=' . $this->request->get['stock_status_id'] . $url, true);
		}

		$data['cancel'] = $this->url->ssl('localisation/stock_status', 'token=' . $this->session->data['token'] . $url, true);
