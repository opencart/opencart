...
class ControllerMarketingMarketing extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->ssl('marketing/marketing', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('marketing/marketing', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->ssl('marketing/marketing/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->ssl('marketing/marketing/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['marketings'][] = array(
				...
				'edit'         => $this->url->ssl('marketing/marketing/edit', 'token=' . $this->session->data['token'] . '&marketing_id=' . $result['marketing_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->ssl('marketing/marketing', 'token=' . $this->session->data['token'] . '&sort=m.name' . $url, true);
		$data['sort_code'] = $this->url->ssl('marketing/marketing', 'token=' . $this->session->data['token'] . '&sort=m.code' . $url, true);
		$data['sort_date_added'] = $this->url->ssl('marketing/marketing', 'token=' . $this->session->data['token'] . '&sort=m.date_added' . $url, true);
		...
		$pagination->url = $this->url->ssl('marketing/marketing', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['marketing_id'])) {
			$data['action'] = $this->url->ssl('marketing/marketing/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->ssl('marketing/marketing/edit', 'token=' . $this->session->data['token'] . '&marketing_id=' . $this->request->get['marketing_id'] . $url, true);
		}

		$data['cancel'] = $this->url->ssl('marketing/marketing', 'token=' . $this->session->data['token'] . $url, true);
