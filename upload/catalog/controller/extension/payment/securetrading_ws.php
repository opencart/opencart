	...
	public function process() {
		...
		if ($order_info) {
			if ($this->config->get('payment_securetrading_ws_3d_secure')) {
				...
				$amount_node = $billing_node->addChild('amount', str_replace(array('.', ','), '', $this->model_extension_payment_securetrading_ws->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'])));
