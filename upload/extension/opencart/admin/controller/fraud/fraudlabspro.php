...
class FraudLabsPro extends \Opencart\System\Engine\Controller {
	...
	public function index() {
		$this->load->language('extension/opencart/fraud/fraudlabspro');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/opencart/fraud/fraudlabspro', 'user_token=' . $this->session->data['user_token'])
		];

		$data['action'] = $this->url->link('extension/opencart/fraud/fraudlabspro', 'user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/opencart/fraud/fraudlabspro', $data));
	}

	public function install() {
		$this->load->model('extension/opencart/fraud/fraudlabspro');

		$this->model_extension_opencart_fraud_fraudlabspro->install();
	}

	public function uninstall() {
		$this->load->model('extension/opencart/fraud/fraudlabspro');

		$this->model_extension_opencart_fraud_fraudlabspro->uninstall();
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/opencart/fraud/fraudlabspro')) {
	...
	public function order() {
		...
		if ($fraud_info) {
			...
			if ($fraud_info['is_bin_found']) {
				$data['flp_bin_found'] = $this->parse_fraud_result($fraud_info['is_bin_found']);
			} else {
				$data['flp_bin_found'] = '';
			}
			...
			if ($fraud_info['is_credit_card_blacklist']) {
				$data['flp_credit_card_blacklist'] = $this->parse_fraud_result($fraud_info['is_credit_card_blacklist']);
			} else {
			...
			return $this->load->view('extension/opencart/fraud/fraudlabspro_info', $data);
		}
	}
