...
class ModelUpgrade1009 extends Model {
	public function upgrade() {
		...
		// url_alias ported to seo_url table. Add multistore and multilanguage
		$query = $this->db->query("SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = '" . DB_DATABASE . "' AND TABLE_NAME = '" . DB_PREFIX . "url_alias'");

		if ($query->num_rows) {
			$url_alias_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "url_alias`");

			$store_query = $this->db->query("SELECT store_id FROM `" . DB_PREFIX . "store`");

			array_unshift($store_query->rows, array('store_id' => '0'));
			...
			foreach ($store_query->rows as $store) {
				foreach ($language_query->rows as $language) {
					foreach ($url_alias_query->rows as $alias) {
						$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url` SET `store_id` = '" . (int)$store['store_id'] . "', `language_id` = '" . (int)$language['language_id'] . "', `query` = '" . $this->db->escape($alias['query']) . "', `keyword` = '" . $this->db->escape($alias['keyword']) . "'");
					}
				}
			}

			$this->db->query("DROP TABLE `" . DB_PREFIX . "url_alias`");
		}
