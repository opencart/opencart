...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		$sqlcmd ="SELECT DISTINCT p.*, pd.*,pd.name AS name, p.image, m.name AS manufacturer, 
				...
				(SELECT GROUP_CONCAT(image) FROM (SELECT image FROM " . DB_PREFIX . "product_saving_image WHERE product_id = " . $product_id . " ORDER BY sort_order ASC) AS saving) as saving_images,
				...
				LEFT JOIN " . DB_PREFIX . "product_dtit pt ON (p.product_id = pt.product_id)
		...
		//	var_dump($query->row);
			return array(
				...
				'saving_images'	=> $query->row['saving_images'],
