...
class CustomerSearch extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/opencart/report/customer_search')
		];

		$data['save'] = $this->url->link('extension/opencart/report/customer_search.save');
	...
	public function getReport(): string {
		...
		foreach ($results as $result) {
			...
			if ($result['customer_id'] > 0) {
				$customer = sprintf($this->language->get('text_customer'), $this->url->link('customer/customer.form', 'customer_id=' . $result['customer_id']), $result['customer']);
			} else {
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $this->config->get('config_pagination'),
			'url'   => $this->url->link('extension/opencart/report/customer_search.report', 'code=customer_search' . $url . '&page={page}')
		]);
