...
class Product extends \Opencart\System\Engine\Model {
	public function addProduct(array $data): int {
		...
		if (isset($data['product_option'])) {
			foreach ($data['product_option'] as $product_option) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "product_option` SET `product_id` = '" . (int)$product_id . "', `option_id` = '" . (int)$product_option['option_id'] . "', `value` = '" . (isset($product_option['value']) ? $this->db->escape($product_option['value']) : '') . "', `required` = '" . (int)$product_option['required'] . "'");
	...
	public function editProduct(int $product_id, array $data): void {
		...
		if (isset($data['product_option'])) {
			foreach ($data['product_option'] as $product_option) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "product_option` SET `product_option_id` = '" . (int)$product_option['product_option_id'] . "', `product_id` = '" . (int)$product_id . "', `option_id` = '" . (int)$product_option['option_id'] . "', `value` = '" . (isset($product_option['value']) ? $this->db->escape($product_option['value']) : '') . "', `required` = '" . (int)$product_option['required'] . "'");
				$product_option_id = $this->db->getLastId();
