			...
			if ($query->num_rows) {
				foreach ($query->rows as $result) {
					...
					if ($result['code'] == $extension['code'] && $result['code'] !=  $extension['type'] . "_" . $extension['code'] && $extension['type'] != "theme") {
						$this->db->query("UPDATE `" . DB_PREFIX . "setting` SET `code` = '" . $extension['type'] . "_" . $extension['code'] . "', `key` = '" . $extension['type'] . "_" . $result['key'] . "', `value` = '" . addslashes($result['value']) . "' WHERE `setting_id` = '" . $result['setting_id'] . "'");
					}
				}
			}	
		}
