...
<table class="table table-striped table-bordered">
	...
	<tr>
		...
		<td id="capture_status">
			...
			<?php } else { ?>
				...
				<?php if ($amazon_login_pay_order['void_status'] == 0) { ?>
	...
	<tr>
		...
		<td id="void_status">
			<?php if ($amazon_login_pay_order['void_status'] == 1) { ?>
	...
	<tr>
		...
		<td id="refund_status">
			...
			<?php } else { ?>
				...
				<?php if ($amazon_login_pay_order['total_captured'] > 0 && $amazon_login_pay_order['void_status'] == 0) { ?>
  ...
  $("#button-void").click(function() {
		if (confirm('<?php echo $text_confirm_void; ?>')) {
			$.ajax({
				...
				url: 'index.php?route=payment/amazon_login_pay/void&token=<?php echo $token; ?>',
				beforeSend: function() {
					$('#button-void').hide();
					$('#loading-void').show();
			...
			$.ajax({
				...
				success: function(data) {
					if (data.error == false) {
						...
						html += '<td class="text-left">' + data.data.type + '</td>';
						html += '<td class="text-left">' + data.data.status + '</td>';
						html += '<td class="text-left">' + data.data.capture_id + '</td>';
				...
				success: function(data) {
					...
					if (data.data != undefined) {
						$.each(data.data, function(index, value) {
							...
							html += '<td class="text-left">' + value['capture_id'] + '</td>';
