  ...
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right">
        <button type="submit" form="form-pilibaba" data-toggle="tooltip" title="<?php echo $button_save; ?>" class="btn btn-primary"><i class="mi mi-save">save</i></button>
        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-pilibaba" class="form-horizontal">
		  ...
		  <div class="tab-content">
		    <div class="tab-pane <?php if ($show_register) { echo 'active'; } ?>" id="tab-register">
			  ...
			  <a class="button btn btn-primary col-sm-offset-2" id="button-register"><?php echo $button_register; ?></a> <span class="btn btn-primary col-sm-offset-2" id="img_loading_register" style="display:none"><i class="mi mi-circle-o-notch mi-spin mi-lg">refresh</i></span>
			</div>
		    <div class="tab-pane <?php if (!$show_register) { echo 'active'; } ?>" id="tab-settings">
		      <?php if ($error_weight) { ?>
                <div class="alert alert-danger"><i class="mi mi-exclamation-circle">error</i> <?php echo $error_weight; ?></div>
			  <?php } ?>

		      <?php if ($notice_email) { ?>
				<div class="alert alert-info"><i class="mi mi-info-circle">info</i> <?php echo $notice_email; ?></div>
			  <?php } ?>
	...
	$('#button-register').click(function() {
		$.ajax({
			...
			success: function(json) {
				...
				if (json['error']) {
					$('#tab-register').prepend('<div class="alert alert-danger pilibaba_message" style="display:none"><i class="mi mi-exclamation-circle">error</i> ' + json['error'] + '</div>').fadeIn();
				}
