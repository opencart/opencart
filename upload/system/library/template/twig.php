...
final class Twig {
	...
	public function render($template, $cache) {
		...
		// 2. Create an anonymous cache class as twig will not all us to generate a key based on
		...
		// 3. Get template class name
		$template_class_name = $twig->getTemplateClass($template . '.twig');

		// 4. Get cache file path . '_' . $namespace
		$cache_file = $twig->getCache(false)->generateKey($template . '.twig', $template_class_name);

		try {
			// 5. Check if cached file exists with modifications if not we create one
			if (!is_file($cache_file)) {
				// 6. Get the source code using the source
				$source = $twig->getLoader()->getSourceContext($template . '.twig');

				$code = $source->getCode();

				// 7. Run the code through the filters
				...
				// 9. Write the output to a cache file
				$twig->getCache(false)->write($cache_file, $output);
			}

			// 10. Load the cached file into array of loaded templates
