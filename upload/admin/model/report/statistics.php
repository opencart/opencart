...
class ModelReportStatistics extends Model {
	...
	public function addValue($code, $value) {
		$this->db->query("UPDATE `" . DB_PREFIX . "statistics` SET `value` = (`value` + '" . $this->db->escape($value) . "') WHERE `code` = '" . $this->db->escape($code) . "'");
	}
	
	public function editValue($code, $value) {
		$this->db->query("UPDATE `" . DB_PREFIX . "statistics` SET `value` = '" . $this->db->escape($value) . "' WHERE `code` = '" . $this->db->escape($code) . "'");
	}
